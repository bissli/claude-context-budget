...
- `--kind`, `--read-before`, `--status`, `--where`, and `--label` each
  ...
  `2a-b ...` is a word. A dotted sub-heading takes its whole number:
  `s24.4` names `### 24.4 The decision`, `s24` names `## 24. Parent`,
  and the title alone lands on the first heading of that text, so a
  repeated title is named by its number. A heading keyed by a
  letter-led id of one or two letters, `## F65. Title` or `## Q3:
  Title`, is named by that id - `F65`, `f65`, or `sF65` - and `F7`
  never lands on `## 7. Seven`; a longer word such as `Log4j:` is title
  text.
...
2. Run `hq open <slug>`. It is read-only and prints only what is wrong;
   ...
   - `stale folder path in <file>: <dir>/<slug>/ x<n>` - the cursor or
     an unsuperseded standing item names the folder under a directory
     an earlier plugin version used, `working/` or `scratch/`, so the
