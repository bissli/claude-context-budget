...
def test_adopt_infers_a_nested_pointer_below_the_top_level(
    ...
    whatever the kind.
    Oracle: the sub/deep.py row - kind other, read_before never, no refusal.
    """
    folder = _root(tmp_path, monkeypatch)
    (folder / 'sub').mkdir()
...
def test_a_traversing_or_empty_slug_is_refused(tmp_path, monkeypatch, capsys):
    ...
    Mutation: the slug joined onto scratch/ unchecked, so `begin ..` turns
...
def test_a_read_only_verb_creates_no_scratch_directory(tmp_path, monkeypatch):
    """A miss on a root with no scratch/ leaves the disk untouched.

    Mutation: scratch.mkdir before the resolve, so any typo on any verb
    creates directories under a wrong root.
    Oracle: root/scratch does not exist after the failed lookup.
    """
...
def test_an_exact_folder_name_beats_a_longer_prefix_match(
    ...
    Oracle: the resolved folder is scratch/demo.
    """
    folder = _root(tmp_path, monkeypatch)
...
def test_the_root_flag_beats_the_environment(tmp_path, monkeypatch, capsys):
    ...
    (other / 'scratch').mkdir(parents=True)
...
def test_a_key_files_label_with_a_trailing_clause_is_structure(
    ...
    _handoff(folder, key_files=(
        'Read now, under `scratch/demo/` unless noted:\n\n'
        '- `notes-a.md` the facts\n'))
    assert hq.main(['adopt', _SLUG]) == 0
    rows = {r['path']: r for r in _ledger(folder)}
    assert rows['notes-a.md']['read_before'] == 'always'
    assert ('- unfiled: Read now, under `scratch/demo/` unless noted:'
